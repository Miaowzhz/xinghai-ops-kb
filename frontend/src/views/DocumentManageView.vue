...
onMounted(() => {
  if (!auth.isAdmin) { ElMessage.error('仅知识管理员可访问'); router.push('/qa'); return }
  loadDocuments()
})
...
<template>
  <div class="manage-page">
    <main class="manage-main">
      <div class="page-title-row"><div class="title-with-back"><button class="back-button" title="返回智能问答" @click="router.push('/qa')"><ArrowLeft :size="18" /></button><div><h1>知识库文档管理</h1><p>管理已入库资料的版本、状态与生命周期。</p></div></div><div class="polling" :class="{ active: polling }"><RefreshCw :size="14" :class="{ spinning: polling }" />{{ polling ? '正在同步入库状态' : '状态已同步' }}</div></div>
      ...
      <section class="table-panel panel">
        ...
        <div v-loading="loading" class="table-wrap">
          ...
          <el-table v-else :data="documents" class="manage-table" row-key="id">
            <el-table-column label="标题" min-width="220"><template #default="{ row }"><el-tooltip :content="row.title" placement="top"><span class="ellipsis title-text">{{ row.title }}</span></el-tooltip></template></el-table-column>
            <el-table-column label="类型" width="110"><template #default="{ row }">{{ typeLabels[row.doc_type] || row.doc_type }}</template></el-table-column><el-table-column prop="product_line" label="产品线" width="100" />
            <el-table-column label="版本" width="130"><template #default="{ row }"><span>{{ row.product_version }}</span><small class="version-count">第 {{ row.version }} 次</small></template></el-table-column><el-table-column label="chunk 数" width="95"><template #default="{ row }">{{ row.chunk_count || 0 }}</template></el-table-column>
            <el-table-column label="状态" width="115"><template #default="{ row }"><el-tooltip v-if="row.status === 'failed'" :content="row.fail_reason || '入库失败，请重试'" placement="top"><el-tag :type="statusType(row.status)" effect="dark">{{ statusLabel(row.status) }}</el-tag></el-tooltip><el-tag v-else :type="statusType(row.status)" effect="dark">{{ statusLabel(row.status) }}</el-tag></template></el-table-column>
            ...
            <el-table-column label="操作" width="200" fixed="right"><template #default="{ row }"><div class="row-actions"><el-button link :loading="detailLoading && detail?.id === row.id" @click="openDetail(row)"><Eye :size="14" />详情</el-button><el-button link :disabled="row.status === 'parsing'" @click="openReingest(row)"><FilePenLine :size="14" />重新上传</el-button><el-tooltip v-if="row.status === 'parsing'" content="入库中，暂不可删除"><span><el-button link disabled><Trash2 :size="14" />删除</el-button></span></el-tooltip><el-button v-else link type="danger" :loading="deletingId === row.id" @click="removeDocument(row)"><Trash2 :size="14" />删除</el-button></div></template></el-table-column>
