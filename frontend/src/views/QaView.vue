...
import { History, LogOut, MessageSquare, MessageSquarePlus, MessageSquareWarning, Send, Settings, ThumbsDown, ThumbsUp, Upload } from '@lucide/vue'
...
const baseNavItems = [
  { name: '智能问答', icon: MessageSquare, path: '/qa' },
  { name: '问答历史记录', icon: History, path: null },
]
const adminNavItems = [
  { name: '知识库文档管理', icon: Settings, path: '/documents/manage' },
  { name: '文档上传与入库', icon: Upload, path: '/documents/upload' },
  { name: '反馈审核工作台', icon: MessageSquareWarning, path: null },
]
...
function navigate(item) {
  ...
  else ElMessage.info(`「${item.name}」功能开发中`)
}
...
<template>
  <div class="qa-page">
    <header class="topbar"><div class="brand"><span class="brand-mark">星</span><span>星海运维智能知识库</span></div><div class="topbar-user"><span>{{ auth.user?.display_name || '用户' }}</span><el-tag effect="dark" type="success">{{ roleLabel }}</el-tag><button title="退出登录" @click="logout"><LogOut :size="16" /></button></div></header>
    <div class="qa-shell">
      <aside class="session-sidebar">
        <nav class="app-navigation"><button v-for="item in navItems" :key="item.name" :class="['app-nav-item', { active: item.name === '智能问答' }]" @click="navigate(item)"><component :is="item.icon" :size="15" /><span>{{ item.name }}</span></button></nav>
...
<style scoped>
.app-navigation{display:flex;flex-direction:column;gap:3px;padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,.07);margin-bottom:16px}.app-nav-item{width:100%;display:flex;align-items:center;gap:9px;border:0;border-radius:6px;background:transparent;color:#829998;padding:9px 10px;text-align:left;font-size:12px;cursor:pointer}.app-nav-item:hover{background:rgba(255,255,255,.04);color:#d8e7e4}.app-nav-item.active{background:rgba(45,212,191,.1);color:#58d2c3}
</style>
